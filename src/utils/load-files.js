...
const isEmptyOneLiner = require('./is-one-liner');
...
const toType = nameParts => {
  ...
  return {
    tType: nameParts[0],
    pType: nameParts >= 1 ? nameParts[1]: null,
    pName: nameParts >= 2 ? nameParts[2]: null
  }
}
const loadFile = async (path) => {
  ...
  for (let i = 0; i < file.lines.clean.length; i += 1) {
    ...
    if (level === 0 && hash.endsWith('}')) {
      node.hash = _.hashLines(trimLines(node.lines));
      node      = null;
      continue;
    }
