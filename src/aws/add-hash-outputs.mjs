...
const addHashOutputs = async (filePath, uidTag = UID_TAG_NAME) => {
  ...
  for (let i = 0; i < resNodes.length; i++) {
    ...
    const hashNodeName = `${resNode.nameHash}_HASH`.toUpperCase();
    
    let arnNode = data.nodes.find(x => x && x.name === hashNodeName);
    if (arnNode?.value) {
      continue;
    };
    ...
    const name = `output(${hashNodeName})`;
    arnNode = {
