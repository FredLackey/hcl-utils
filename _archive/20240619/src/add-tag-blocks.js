...
const isReourceMatch = require('./utils/is-resource-match');
const isTaggable = require('./utils/is-taggable');
const isGoogle = require('./utils/is-google');
const isResourceMatch = require('./utils/is-resource-match');
...
const updateTags = (content, resourceName, tagKey, tagValue, wrapInQuotes) => {
  ...
  let updatedLines = lines.map((line, index) => {
    ...
    if (!isResourceMatch(line, resourceName)) {
    ...
    if (insideResource && isTaggable(resourceType)) {
      if (isGoogle(resourceType) && lineHash.startsWith('labels={')) {
        insideTagBlock = true;
      } else if (!isGoogle(resourceType) && lineHash.startsWith('tags={')) {
        insideTagBlock = true;
      }
      ...
      if (insideTagBlock) {
        if (trimmedLine === '}' || trimmedLine === '],') {
          ...
          const formattedValue = wrapInQuotes ? `"${tagValue}"` : tagValue;
          return line.replace(new RegExp(`(${tagKey} =) "[^"]*"`, 'g'), `$1 ${formattedValue}`);
        }
      }
